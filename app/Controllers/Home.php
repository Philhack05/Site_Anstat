...
class Home extends BaseController
{
    ...
    public function store()
    {
        ...
        //return $this->response->setJSON($this->request->getPost());
            ...
            $api = new AnstatApi();
            $value = $this->request->getPost('B4_1');  
            list($cod_dep, $cod_sp) = explode('_', $value);
            $getOneDistrict = $api->getOneDistrict($this->request->getPost('B1'));
            $getOneRegion = $api->getOneRegion($this->request->getPost('B2'));
            $getOneDept = $api->getOneDept($cod_dep);
            $getOneSp = $api->getOneSp($cod_dep,$cod_sp);
             // Récupération de la valeur envoyée depuis le formulaire
            
            $getOneLoc = $api->getOneLoc($this->request->getPost('B7a'),$cod_dep,$cod_sp);
            $now = Time::now();
            ...
            $dataPDF = [      
                ...
                'Quelles sont les principales contraintes que votre entreprise rencontre dans le cadre votre activité (choix multiples) ? (coché par ordre d’importance)'=>':'
            ];
            ...
            foreach ($etablissements as $key => $et) {
                $dataPDFEt[$key] = [ 
                    'Raison Social'          => $et,
                    'Adresse géographique'            => $adresses[$key],
                    'Localité'          => $localites[$key],  
            ...
            $datargeci = [
                ...
                'datecollecte_deb'      => $this->request->getPost('datecollecte_deb'), //$now->toDateTimeString(),
                'datecollecte_fin'      => $this->request->getPost('datecollecte_fin'), //$now->toDateTimeString(),
                'b8'                    => $this->request->getPost('B8'),
                'b8aut'                 => null,
